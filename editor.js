...
async function loadFromGitHub() {
  try {
    ...
    for (const language of LANGUAGES) {
      ...
      if (!response.ok) throw new Error(githubErrorMessage(response, payload, `завантажити ${filePath(language)}`));
...
function githubErrorMessage(response, payload, action) {
  const details = payload?.message || `${response.status} ${response.statusText}`;
  const accepted = response.headers.get('x-accepted-github-permissions');
  const suffix = accepted ? ` Потрібні дозволи: ${accepted}.` : '';

  if (response.status === 401) return `Токен недійсний або прострочений. ${details}${suffix}`;
  if (response.status === 403) return `GitHub заборонив ${action}. Перевірте доступ token до репозиторію, Contents: Read and write, правила гілки та ліміт API. ${details}${suffix}`;
  if (response.status === 404) return `Файл або репозиторій не знайдено. Перевірте owner, repo, branch і шлях «${filePath('uk').replace('/uk.json', '')}». Для приватного репозиторію 404 також означає, що token не має доступу. ${details}`;
  if (response.status === 409) return `Конфлікт під час запису. Файл або гілка змінилися, або гілка порожня. ${details}`;
  if (response.status === 422) return `GitHub відхилив дані. Найчастіше це неправильний SHA, назва гілки або правило захисту гілки. ${details}`;
  return `Не вдалося ${action}: ${details}${suffix}`;
}

async function getCurrentFile(language) {
  ...
  const payload = await readJsonResponse(response);
  if (!response.ok) throw new Error(githubErrorMessage(response, payload, `прочитати ${filePath(language)}`));
  if (!payload.sha || payload.type !== 'file') throw new Error(`${filePath(language)} не є звичайним файлом GitHub.`);
  return payload;
}

async function updateGitHubFile(language) {
  // SHA має бути актуальним безпосередньо перед PUT, інакше GitHub поверне 409/422.
  const current = await getCurrentFile(language);
  ...
  const payload = await readJsonResponse(response);
  if (!response.ok) throw new Error(githubErrorMessage(response, payload, `опублікувати ${filePath(language)}`));
  state.shas[language] = payload.content?.sha || null;
